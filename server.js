...
async function hGetContests(env,uid,_data,_meta={}){
  try{
    ...
    for(const [id,u] of Object.entries(users)){
      ...
      // Referrals contest: count ONLY referrals whose USER PROFILE has hasDeposited === true
      let refCount=0;
      const refs=u.referrals||{};
      for(const r of Object.values(refs)){
        if(!r)continue;
        // Look up the referred user's actual profile to check hasDeposited
        const refUserId=r.userId||r.id||null;
        const refUser=refUserId?users[refUserId]:null;
        if(refUser&&refUser.hasDeposited===true)refCount++;
      }
    ...
    if(q.data){
      ...
      if(waitedMs>=BOT_MATCH_TIMEOUT && waitedMs<RACE_MATCH_TTL){
        ...
        const match={
          ...
          winnerUid,prize:RACE_PRIZE,cost:RACE_COST,
          isBot:true,botUid,
          // User is p1 (left HUD = user speed), bot is p2 (right HUD = bot speed)
          // The game reads p1.maxKmh for the left/you display and p2.maxKmh for the right/opp display.
          // Keeping user=p1 ensures speeds show correctly without any HUD mismatch.
        ...
        return{success:true,data:{
          ...
          youAreP1:true,prize:RACE_PRIZE,   // user is p1; bot is p2
...
app.get('/tonconnect-manifest.json', (req, res) => {
  res.set(CORS).json({ url: 'https://raseenracing.vercel.app', name: 'Rassen Racing', iconUrl: 'https://res.cloudinary.com/dktppfipy/image/upload/v1779047569/icon_gwkepe.jpg', description: 'Raseen Racing Garage' });
});
